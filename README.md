...
| --------- | ------------------------------------------------------------- | ----------------------------------------------------- | ------------------------------------ |
| Windows   | `winws` + драйвер WinDivert, служба `senboost-zapret`         | UAC при первой настройке                              | стабильно, гарантированно работает   |
...
```bash
sudo "/Library/Application Support/SenBoost/zapret/stop.sh"
```
...
как есть, а не блокируется), но после перезагрузки заново их никто не поднимает. Автозапуск
самого приложения (см. ниже) частично это закрывает — обход поднимется при входе в систему,
но потребует `pkexec` на каждый вход.

## Планы до 1.0

- [ ] Очистка и ревью нейросгенерированного кода
- [ ] Полная поддержка MacOS и Linux
- [x] Фоновая работа в трей
- [ ] Поддержка Telegram-прокси
